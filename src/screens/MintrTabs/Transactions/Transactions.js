				...
				const response = await Promise.all([
					fetch(
						`${getApiUrl(networkName)}/blockchainEventsFiltered${stringifyQuery({
							fromAddress: hexifyAddress(walletAddress),
						})}`
					),
					/*fetch(
						`${getApiUrl(networkName)}/blockchainEventsFiltered${stringifyQuery({
							toAddress: hexifyAddress(walletAddress),
...
const getEventInfo = data => {
	...
	switch (event) {
		...
		case 'ClearedDeposit':
			...
			break;
		case 'Exchange':
			if (data.exchangeFromCurrency === 'TRX') {
			...
			<Table cellSpacing="0">
				...
				<TBody>
					{data
						.filter(dataElement => {
							return (
								dataElement.event !== 'Approval' &&
								dataElement.event !== 'Transfer' &&
								dataElement.event !== 'InversePriceFrozen' &&
								dataElement.event !== 'IssuanceDebtRatioEntry' &&
								dataElement.event !== 'OracleUpdated' &&
								dataElement.event !== 'TargetUpdated'
							);
						})
						.map((dataElement, i) => {
								...
								<TR key={i}>
									...
									<TD style={{ textAlign: 'right' }}>
										<BorderlessButton
											...
											}tronscan.io/#/transaction/${dataElement.transactionHash}`}
											as="a"
											target="_blank"
										>
