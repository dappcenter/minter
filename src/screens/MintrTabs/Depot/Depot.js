							...
							<HiddenTableRow key={i}>
								...
								<HiddenTableCell>
									<BorderlessButton
										...
										}tronscan.io/#/transaction/${detail.transactionHash}`}
										as="a"
										target="_blank"
									>
	...
	useEffect(() => {
		const getDepotEvents = async () => {
			try {
				...
				const [depositsMade, depositsRemoved] = await Promise.all(
					results.map(response => response.json())
				);
				const depositsCleared = [];
	...
	useEffect(() => {
		const getDepotData = async () => {
			try {
				...
				//console.log("set Data to", totalSellableDeposits, minimumDepositAmount, sUSDBalance )
			} catch (e) {
				console.log(e);
...
const Depot = ({ t }) => {
	...
	//console.log("got amountAvailable", amountAvailable, "depositsMade", depositsMade)
				...
				<ActivityHeader>
					...
					<MoreButtons>
						...
						<ButtonTertiary
							href={`https://${
								networkName === 'mainnet' ? '' : networkName + '.'
							}tronscan.io/#/address/${base58address(snxJSConnector.snxJS.Depot.contract.address)}`}
							as="a"
							target="_blank"
						>
