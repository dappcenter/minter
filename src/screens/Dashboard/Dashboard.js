...
const getBalancePerAsset = (asset, { balances, prices, debtData, synthData }) => {
	...
	//console.log(balances, prices, debtData, synthData)
...
const processTableData = (state, t) => {
	return ['OKS', 'sUSD', 'TRX', 'Synths', 'Debt'].map(dataType => {
		let iconName = ['Synths', 'Debt'].includes(dataType) ? 'oks' : dataType;
		...
		iconName = ['sUSD'].includes(iconName) ? iconName : iconName.toUpperCase();
		return {
			rowLegend: (
				<TableIconCell>
				...
				<Container>
					...
					<PricesContainer>
						{['OKS', 'TRX'].map(asset => {
							return (
								<Asset key={asset}>
									<CurrencyIcon src={`/images/currencies/${asset.toUpperCase()}.svg`} />
									{isEmpty(prices) ? (
