			...
			try {
				...
				const getBalanceForWallet = async wallet => {
					return {
						...
						trxBalance: await snxJSConnector.provider.getBalance(wallet.address),
				...
				nextWallets.forEach((wallet, index) => {
					getBalanceForWallet(wallet, index).then(balance => {
						wallet.balances = {
							...
							trxBalance: snxJSConnector.utils.formatTron(balance.trxBalance),
							//ethBalance: snxJSConnector.utils.formatTron(balance.ethBalance),
						};
							...
							{!isLoading ? (
								<ListInner>
									<List cellSpacing={0}>
										<ListHead>
											<ListHeaderRow>
												{['Address', 'OKS', 'sUSD', 'TRX', ''].map((headerElement, i) => {
													return (
														<ListHeaderCell
												...
												.map((wallet, i) => {
													return (
														...
														>
															...
															<ListCell style={{ textAlign: 'right' }}>
																<BalanceOrSpinner value={wallet.balances.trxBalance} />
															</ListCell>
															<ListCell
															...
															>
																<Link
																	...
																	}tronscan.io/#/address/${wallet.address}`}
																	target="_blank"
																>
																	<LinkImg width="20" src="/images/tronscan-logo.png" />
																</Link>
															</ListCell>
														</ListBodyRow>
													);
